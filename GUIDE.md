...
**Почему SQLite?** Не требует установки отдельного сервера БД. Файл `app.db` — это вся база. Удобно для разработки и обучения. В продакшене обычно используют SQL Server или PostgreSQL.
...
**DbContext** — это «сессия» работы с БД. Он знает о всех сущностях и их связях. `OnModelCreating` нужен для настройки ограничений (длина строк, точность decimal) и связей между таблицами.
...
**Unit of Work** — «пакет операций». Если нужно добавить категорию и несколько товаров, все они сохраняются одним вызовом. При ошибке откатывается вся транзакция.
...
**REST** — стиль проектирования API. Ресурсы (products, categories) представлены URL. HTTP-методы (GET, POST, PUT, DELETE) обозначают действие. Код ответа (200, 201, 404) описывает результат.
...
**Seed** — «начальные данные». Нужны, чтобы сразу после запуска можно было протестировать API, не добавляя данные вручную.
